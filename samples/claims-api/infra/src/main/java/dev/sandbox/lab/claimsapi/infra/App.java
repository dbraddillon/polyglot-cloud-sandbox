package dev.sandbox.lab.claimsapi.infra;
...
public class App {
    private static final String DB_NAME = "claims";
    private static final String DB_USER = "claims";
    private static final String DB_PASSWORD = "claims";
    ...
    public static void main(String[] args) {
        Pulumi.run(ctx -> {
            ...
            var network = new Network("claimsNetwork", NetworkArgs.builder()
                    .name("claims-network")
                    .build());

            var db = new Container("claimsDb", ContainerArgs.builder()
                    .name("claims-db")
            ...
            var image = new Image("claimsApiImage", ImageArgs.builder()
                    .imageName("claims-api:local")
            ...
            // "claims-db") is what makes Pulumi start the database before the app - the same
            ...
            var app = new Container("claimsApiApp", ContainerArgs.builder()
                    .name("claims-api-app")
                    // repoDigest, not imageName: imageName is the mutable tag ("claims-api:local")
            ...
            ctx.export("appUrl", Output.of("http://localhost:8083/claims"));
        });
    }
}
